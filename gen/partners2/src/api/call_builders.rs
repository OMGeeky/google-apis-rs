...
/// # }
/// ```
pub struct MethodGetPartnersstatusCall<'a, S>
    where S: 'a {
...
impl<'a, S> client::CallBuilder for MethodGetPartnersstatusCall<'a, S> {}

impl<'a, S> MethodGetPartnersstatusCall<'a, S>
where
    S: tower_service::Service<http::Uri> + Clone + Send + Sync + 'static,
    S::Response: hyper::client::connect::Connection + AsyncRead + AsyncWrite + Send + Unpin + 'static,
    S::Future: Send + Unpin + 'static,
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    ///
    /// Sets the *request metadata.user overrides.user id* query property to the given value.
    pub fn request_metadata_user_overrides_user_id(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_user_overrides_user_id = Some(new_value.to_string());
        self
    }
    /// IP address to use instead of the user's geo-located IP address.
    ///
    /// Sets the *request metadata.user overrides.ip address* query property to the given value.
    pub fn request_metadata_user_overrides_ip_address(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_user_overrides_ip_address = Some(new_value.to_string());
        self
    }
    /// Second level identifier to indicate where the traffic comes from.
    ...
    ///
    /// Sets the *request metadata.traffic source.traffic sub id* query property to the given value.
    pub fn request_metadata_traffic_source_traffic_sub_id(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_traffic_source_traffic_sub_id = Some(new_value.to_string());
        self
    }
    /// Identifier to indicate where the traffic comes from.
    ...
    ///
    /// Sets the *request metadata.traffic source.traffic source id* query property to the given value.
    pub fn request_metadata_traffic_source_traffic_source_id(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_traffic_source_traffic_source_id = Some(new_value.to_string());
        self
    }
    /// Google Partners session ID.
    ///
    /// Sets the *request metadata.partners session id* query property to the given value.
    pub fn request_metadata_partners_session_id(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_partners_session_id = Some(new_value.to_string());
        self
    }
    /// Locale to use for the current request.
    ///
    /// Sets the *request metadata.locale* query property to the given value.
    pub fn request_metadata_locale(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_locale = Some(new_value.to_string());
        self
    }
    /// Experiment IDs the current request belongs to.
    ...
    /// Append the given value to the *request metadata.experiment ids* query property.
    /// Each appended value will retain its original ordering and be '/'-separated in the URL's parameters.
    pub fn add_request_metadata_experiment_ids(mut self, new_value: &str) -> MethodGetPartnersstatusCall<'a, S> {
        self._request_metadata_experiment_ids.push(new_value.to_string());
        self
    }
    /// The delegate implementation is consulted whenever there is an intermediate result, or if something goes wrong
    ...
    ///
    /// Sets the *delegate* property to the given value.
    pub fn delegate(mut self, new_value: &'a mut dyn client::Delegate) -> MethodGetPartnersstatusCall<'a, S> {
        self._delegate = Some(new_value);
        self
    }
    ...
    /// * *oauth_token* (query-string) - OAuth 2.0 token for the current user.
    /// * *callback* (query-string) - JSONP
    pub fn param<T>(mut self, name: T, value: T) -> MethodGetPartnersstatusCall<'a, S>
                                                        where T: AsRef<str> {
        self._additional_params.insert(name.as_ref().to_string(), value.as_ref().to_string());
        self
    }
...
///              .view(&CompanyViewEnum::COMPANYVIEWUNSPECIFIED)
...
///              .website_url("sed")
///              .view(&CompanyViewEnum::COMPANYVIEWUNSPECIFIED)
///              .add_specializations(&CompanySpecializationsEnum::BADGESPECIALIZATIONUNKNOWN)
///              .add_services(&CompanyServicesEnum::SERVICEUNSPECIFIED)
...
///              .add_industries(&CompanyIndustriesEnum::INDUSTRYUNSPECIFIED)
///              .add_gps_motivations(&CompanyGpsMotivationsEnum::GPSMOTIVATIONUNSPECIFIED)
///              .company_name("elitr")
///              .address("sed")
///              .doit().await;
/// # }
...
///              .user_view(&UserUserViewEnum::BASIC)
