...
impl<'a, S> client::Hub for Spanner<S> {}

impl<'a, S> Spanner<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> Spanner<S> {
        Spanner {
            client,
            auth: Box::new(auth),
    ...
    pub fn scans(&'a self) -> ScanMethods<'a, S> {
