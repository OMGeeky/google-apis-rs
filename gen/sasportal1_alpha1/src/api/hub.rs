...
impl<'a, S> client::Hub for Sasportal<S> {}

impl<'a, S> Sasportal<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> Sasportal<S> {
        Sasportal {
            client,
            auth: Box::new(auth),
