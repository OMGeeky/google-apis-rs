...
///              .graceful_decommission_timeout(chrono::Duration::seconds(6569141))
///              .doit().await;
/// 
/// match result {
///     Err(e) => match e {
...
impl<'a, S> client::Hub for Dataproc<S> {}

impl<'a, S> Dataproc<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> Dataproc<S> {
        Dataproc {
            client,
            auth: Box::new(auth),
