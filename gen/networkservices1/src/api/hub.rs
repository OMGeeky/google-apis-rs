...
impl<'a, S> client::Hub for NetworkServices<S> {}

impl<'a, S> NetworkServices<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> NetworkServices<S> {
        NetworkServices {
            client,
            auth: Box::new(auth),
